...
export default function JobDetail() {
  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        ...
        setJob(res.data.data); // Data lấy từ backend
      } catch (err) {
  ...
  const getLogoUrl = (url: string) => {
    if (!url) return "https://ui-avatars.com/api/?name=Company&background=random"; // Thay via.placeholder vì hay bị lỗi mạng
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm">
              <div className="flex flex-col sm:flex-row justify-between gap-6">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-gray-500">
                    <div className="flex items-center gap-1.5 bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider">
                      {/* Lấy trực tiếp job.company_name từ SQL thuần */}
                      <Building className="w-4 h-4" /> {job.company_name || 'Verified Company'}
                    </div>
                    {/* Lấy trực tiếp job.location_name từ SQL thuần */}
                ...
                <button className="bg-blue-600 text-white px-8 py-3.5 rounded-full font-bold shadow-lg hover:bg-blue-700 transition-all h-fit active:scale-95">
                  Apply Now
                </button>
              </div>
            </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              ...
              <div className="flex items-center gap-4 mb-6 group">
                <div className="w-14 h-14 rounded-xl overflow-hidden border border-gray-100 bg-white shadow-sm flex-shrink-0">
                  <img 
                    // Lấy trực tiếp job.logo_url thay vì company.logo_url
                ...
                <div>
                  {/* Lấy trực tiếp job.company_name */}
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">SHARE THIS JOB</h3>
              <div className="flex gap-3">
                <button className="flex-1 flex justify-center py-2.5 bg-blue-50 text-blue-400 rounded-xl hover:bg-blue-100 transition-colors"><Twitter className="w-5 h-5" /></button>
                <button className="flex-1 flex justify-center py-2.5 bg-blue-50 text-blue-700 rounded-xl hover:bg-blue-100 transition-colors"><Linkedin className="w-5 h-5" /></button>
                <button className="flex-1 flex justify-center py-2.5 bg-gray-50 text-gray-500 rounded-xl hover:bg-gray-100 transition-colors"><LinkIcon className="w-5 h-5" /></button>
              </div>
            </div>

             {/* Related Jobs - Mình giữ nguyên mock data nhé */}
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              <h3 className="text-lg font-bold text-gray-900 mb-5">Related Jobs</h3>
              <div className="space-y-6">
                {[
                  ...
                  <div key={i} className="group cursor-pointer">
                    <h4 className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors">{rj.title}</h4>
                    <p className="text-sm text-gray-400 mt-1 font-medium">{rj.comp} • {rj.loc}</p>
                  </div>
                ))}
              </div>
            </div>
