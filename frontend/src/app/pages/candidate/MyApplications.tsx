    ...
    const fetchApplications = async () => {
      try {
        ...
        if (result.success && Array.isArray(result.data)) {
          // Xử lý dữ liệu linh hoạt hơn để không bị lỗi khi thiếu thông tin
          const formattedData = result.data.map((item: any) => ({
            ...
            appliedDate: item.appliedDate ? new Date(item.appliedDate).toLocaleDateString('vi-VN') : 'N/A',
            status: item.status || 'pending', // Mặc định là pending nếu Backend chưa trả về status
            logoUrl: item.logoUrl || 'https://via.placeholder.com/150?text=Job' // Ảnh dự phòng để tránh lỗi kết nối
          }));
