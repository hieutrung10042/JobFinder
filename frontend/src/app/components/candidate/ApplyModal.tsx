// ApplyModal.tsx  — tích hợp API POST /api/applications/apply
...
interface ApplyModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void; // callback để refresh MyApplications
}

type Step = 1 | 2 | 3 | 4;

interface FormState {
  cv_snapshot_url: string; // URL sau khi upload hoặc từ profile
  cvFileName: string;
  cover_letter: string;
}
...
// Giả lập CV từ profile (thực tế lấy từ API /api/profile)
const PROFILE_CV = {
  url: 'https://storage.example.com/resumes/john_doe_resume_2024.pdf',
...
export default function ApplyModal({ job, isOpen, onClose, onSuccess }: ApplyModalProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // TODO: thực tế → upload lên S3/Cloudinary, lấy về URL
    // const fd = new FormData(); fd.append('file', file);
    // const { data } = await axios.post('/api/upload/cv', fd);
    // const url = data.url;
  ...
  const selectProfileCV = () => {
    setUseProfileCV(true);
    setForm(prev => ({ ...prev, cv_snapshot_url: PROFILE_CV.url, cvFileName: PROFILE_CV.name }));
  };

  // ── submit ────────────────────────────────────────────────────────────────
  const handleSubmit = async () => {
    const cvUrl = useProfileCV ? PROFILE_CV.url : form.cv_snapshot_url;
    if (!cvUrl) { toast.error('Please select or upload a CV'); return; }

    setLoading(true);
    try {
      const { data } = await axios.post(
        '/api/applications/apply',  // route hiện có
        {
          job_id:       job.id,
      ...
      if (data.success) {
        ...
        onSuccess?.();   // refresh danh sách ở MyApplications
        reset();
        onClose();
      } else {
  ...
  const canProceed = () => {
    if (step === 1) return useProfileCV ? true : !!form.cv_snapshot_url;
    return true;
  };
      ...
      >
            ...
            <React.Fragment key={n}>
              <div
                className={`flex-none w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-colors
                  ...
                  : step === n ? 'bg-blue-600 text-white ring-4 ring-blue-100 dark:ring-blue-900'
            ...
            <div className="space-y-3 mt-2">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-900 dark:text-white truncate">{PROFILE_CV.name}</p>
                  <p className="text-xs text-gray-500 mt-0.5">Last updated: {PROFILE_CV.lastUpdated} (From Profile)</p>
                </div>
              ...
              <button
                ...
                className={`w-full flex items-center gap-4 p-4 rounded-xl border-2 border-dashed text-left transition-all
                  ${!useProfileCV && form.cv_snapshot_url
                    ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20'
            ...
            <div className="mt-2 space-y-3">
              <div className="bg-gray-50 dark:bg-slate-900 rounded-xl p-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-slate-400">Job</span>
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-slate-400">CV</span>
                  <span className="font-semibold text-gray-900 dark:text-white truncate max-w-[200px]">
                    {useProfileCV ? PROFILE_CV.name : form.cvFileName}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-slate-400">Cover Letter</span>
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-gray-50 dark:bg-slate-900 border-t border-gray-100 dark:border-slate-700">
          <button
            ...
            disabled={loading}
            className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white disabled:opacity-40 transition-colors"
          >
            {step === 1 ? 'Cancel' : '← Back'}
          </button>
